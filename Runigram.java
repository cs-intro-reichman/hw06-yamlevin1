...
public class Runigram {
	...
	public static Color[][] read(String fileName) {
		...
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[0].length; j++) {
				Color threeColors = new Color(in.readInt(), in.readInt(), in.readInt());
				image[i][j] = threeColors;
			}
		}
		return image;
	}
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < flippedHorizontally.length; i++) {
			for (int j = 0; j < flippedHorizontally[0].length; j++) {
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = 0; i < flippedVertically.length; i++) {
			for (int j = 0; j < flippedVertically[0].length; j++) {
	...
	public static Color luminance(Color pixel) {
		int redGrey = (int) (0.299 * pixel.getRed() + 0.587 * pixel.getGreen() + 0.114 * pixel.getBlue());
		int greenGrey = (int) (0.299 * pixel.getRed() + 0.587 * pixel.getGreen() + 0.114 * pixel.getBlue());
		int blueGrey = (int) (0.299 * pixel.getRed() + 0.587 * pixel.getGreen() + 0.114 * pixel.getBlue());
		Color greyColors = new Color(redGrey, greenGrey, blueGrey);
		return greyColors;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < scaledImage.length; i++) {
			for (int j = 0; j < scaledImage[0].length; j++) {
				double row = i * (double) image.length / height;
				double col = j * (double) image[0].length / width;
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color resultColor = new Color(blendRed, blendGreen, blendBlue);
		return resultColor;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		int index = 0;
		while (index < n) {
			double alpha = (double)((n - index) / (double)n);
